...
function SkillTags(props: any) {
    ...
    const removeTheSkill = ()=>{
        ...
        if(props.type==="project"){
            handleAddProjectChange(`${props.type}_tags`, postTags.filter((tag: any)=>tag!==props.id));
        }else{
            handleAddIdeaChange(`${props.type}_tags`, postTags.filter((tag: any)=>tag!==props.id));
        }
    }
    return (
        <div className="skills-tag">
            <label>{props.skill}</label>
            <button onClick={removeTheSkill}>
                &times;
            </button>
        </div>
    )

...
function ModalPart2(props: any) {
    ...
    const addAnotherRole = (role: any, tags: any) => {
        ...
        setAllRolesNeeded([
            ...allRolesNeeded,
            { role: role, skillNeeded: tags },
        ])
    }
    const removeTheRole = (id: any) => {
        setAllRolesNeeded(
            allRolesNeeded.filter((ele: any, idx: any) => idx !== id)
        )
    }
    ...
    const searchUsernames = (e: any) => {
        ...
        setSearchedPeople([
            ...peopleYouMayKnow
                .filter((ppl: any) => ppl.user_name.includes(value))
                .slice(0, 4),
        ])
    }
    ...
    const addMentions = (mention: any) => {
        ...
        setSearchedPeople([])
        setMentionsOfProject([
            ...mentionsOfProject.filter((men: any) => {
    ...
    const searchTeam = (e: any) => {
        ...
        setTeamSearched(
            userTeams.filter((team: any) => team.name.includes(e.target.value))
        )
    }
    ...
    return (
        <div className="modal_part_two">
                    ...
                    <div
                        onClick={(e) => {
                            if (isIndividualProject)
                                handleAddProjectChange('team_id', null)
                            teamMentionInput.current.value = ''
                        }}
                    >
                        <input
                            type="checkbox"
                            onClick={toggleIsIndividualProject}
                            id="individual"
                        />
                    ...
                    {!isIndividualProject && (
                        <div>
                            <input
                                type="checkbox"
                                id="team-not-inovact"
                                onClick={toggleTeamNotOnInovact}
                            />
                            <label
                                htmlFor="team-not-inovact"
                                onClick={toggleTeamNotOnInovact}
                            >
                                Team not on Inovact
                            </label>
            ...
            {!isIndividualProject && !teamNotOnInovact && (
                <div className="modal_part_two-mentions">
                    <label>Mentions</label>
                    <input
                        type="text"
                        placeholder="Type the usernames of the people you would like to mention"
                        onChange={searchUsernames}
                        ref={mentionInput}
                    />
                        ...
                        <div>
                            <div onClick={changeProjectStatus}>
                                <input
                                    ...
                                    name="project-status"
                                    id="just-started"
                                    data-value="just_started"
                                />
                                <label
                                    htmlFor="just-started"
                                    data-value="just_started"
                                >
                                    Just Started
                                </label>
                            ...
                            {allRolesNeeded.map((ele: any, index: any) => {
                                ...
                                return (
                                    <RolesLookingFor
                                        ...
                                        removeTheRole={removeTheRole}
                                        id={index}
                                    />
                                )
                            })}
                            <AddRolesLookingFor
                                addAnotherRole={addAnotherRole}
                            />
                        </div>
                    )}
                </div>

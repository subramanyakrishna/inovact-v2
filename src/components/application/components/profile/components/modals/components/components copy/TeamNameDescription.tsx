...
function TeamNameDescription(props: any) {
    ...
    const loadFile = async(e: any)=>{
        imageUploader(e.target.files).then((data: any)=>{
            handleAddProjectChange("documents", [...addProject.documents, ...data.map((img: any)=>img.url)]);
        });
    }
    const removeImage = (image: any)=>{
        handleAddProjectChange("documents",addProject.documents.filter((ele: any)=>ele!==image));
    }
    ...
    return (
        <div className="modal_part_one-team-description">
            <div className="modal_part_one-title">
                ...
                <input type="text" placeholder="Give your project a suitable title" value={addProject.title} />
            </div>
            <div className="modal_part_one-description">
                <label >Project Description</label>
                <textarea placeholder="Describe your project" value={addProject.description} onChange={(e: any)=>handleAddProjectChange("description", e.target.value)}/>
                <div>

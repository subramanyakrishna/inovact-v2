...
function TeamMemberTag(props: any) {
    ...
    const connectedPeopleAccountIds = useSelector(
    ...
    return (
        <div className="post-dedicated-team-members">
            <div className="post-dedicated-team-members-user">
                ...
                <p className="post-dedicated-team-members-user-name">
                    {props.name}
                </p>
            </div>
            {connectedPeopleAccountIds.includes(props.id) &&
                props.id !== user_id && (
                    <button
                        className="post-dedicated-team-members-btn"
                        style={{ color: isRequested ? '#5579bd' : 'grey' }}
                        onClick={toggleIsRequested}
                    >
                        {isRequested ? 'Requested' : 'Connect'}
                    </button>
                )}
            <p className="post-dedicated-team-members-role">
                {props.role ? props.role : 'Role'}
            </p>
        </div>
    )
}
